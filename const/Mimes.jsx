export default class Mimes {
    ...
    static guessTypeFromExtension(extension) {
        ...
        if (!Object.prototype.hasOwnProperty.call(Mimes.mimes, ext)) return null;
        const val = Mimes.mimes[ext];
     ...
     * @returns {string|null}
     */
    static guessExtensionFromType(type, proposedExtension = null) {
        const t = type.trim().toLowerCase();
        const proposed = (proposedExtension ?? "").trim().toLowerCase().replace(/^\./, "");

        if (proposed !== "" && Object.prototype.hasOwnProperty.call(Mimes.mimes, proposed)) {
            const val = Mimes.mimes[proposed];
            if ((Array.isArray(val) ? val : [val]).includes(t)) return proposed;
        }

        for (const [ext, types] of Object.entries(Mimes.mimes)) {
            if ((Array.isArray(types) ? types : [types]).includes(t)) return ext;
        }
    ...
    /**
     * Returns all mime types associated with an extension.
     * @param {string} extension
     * @returns {string[]}
     */
    static getAllTypesFromExtension(extension) {
        const ext = extension.trim().toLowerCase().replace(/^\./, "");
        if (!Object.prototype.hasOwnProperty.call(Mimes.mimes, ext)) return [];
        const val = Mimes.mimes[ext];
        return Array.isArray(val) ? val : [val];
    }

    /**
     * Returns all extensions associated with a mime type.
     * @param {string} type
     * @returns {string[]}
     */
    static getAllExtensionsFromType(type) {
        const t = type.trim().toLowerCase();
        return Object.entries(Mimes.mimes)
            .filter(([, types]) => (Array.isArray(types) ? types : [types]).includes(t))
            .map(([ext]) => ext);
    }
}